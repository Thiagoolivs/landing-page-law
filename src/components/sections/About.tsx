        ...
        <div className="grid items-center gap-16 lg:grid-cols-2">
          ...
          <Reveal direction="right" className="relative order-2 lg:order-1">
            <div className="relative mx-auto max-w-md lg:max-w-none">
              <div
                className="absolute -left-4 -top-4 h-full w-full rounded-md border border-navy-800/25 dark:border-navy-300/25"
                aria-hidden="true"
              />
              <div className="relative flex aspect-[4/5] items-center justify-center overflow-hidden rounded-md bg-ink-950">
                <div
                  className="absolute inset-0 bg-[radial-gradient(ellipse_70%_60%_at_50%_30%,rgba(144,163,193,0.16),transparent_65%)]"
                  aria-hidden="true"
                />
                <Scale
                  className="h-40 w-40 text-navy-300/80 sm:h-52 sm:w-52"
                  strokeWidth={0.75}
                  aria-hidden="true"
                />
                <div className="absolute inset-x-0 bottom-0 border-t border-white/10 bg-ink-950/80 p-6 backdrop-blur">
                  ...
                  <p className="mt-2 text-xs uppercase tracking-[0.25em] text-navy-300">
                    Ulpiano — Digesto
                  </p>
                </div>
              </div>
              <div className="absolute -bottom-6 -right-4 rounded-md border border-ink-100 bg-white px-6 py-4 shadow-xl dark:border-ink-700 dark:bg-ink-800 sm:-right-8">
                <p className="font-serif text-4xl font-semibold text-navy-800 dark:text-navy-300">
                  15+
                </p>
              ...
              {VALUES.map((value, index) => (
                <Reveal key={value.title} delay={0.2 + index * 0.1}>
                  <li className="flex gap-4">
                    <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-sm border border-navy-800/20 bg-navy-800/[0.06] dark:border-navy-300/25 dark:bg-navy-300/10">
                      <value.icon
                        className="h-5 w-5 text-navy-800 dark:text-navy-300"
                        aria-hidden="true"
                      />
                    </span>
                    <div>
